#!/usr/bin/env python3
"""Plan or execute an isolated UE 5.7 import of the sealed CC0 R14 actions.
...
fresh BuildPlugin package containing the R14 closed editor bridge.  The fixed
source candidate, worker receipt, R3 character project, engine, and bubblewrap
...
from tools.ue.vista_playable_home import (
    makehuman_cc0_detail_actions_r14_contract as commandlet_contract,
)


PLAN_SCHEMA = "vista.makehuman-cc0-r14-ue57-dev-import-plan/v1"
HOST_SCHEMA = "vista.makehuman-cc0-r14-ue57-dev-import-host/v1"
COMPLETE_STATUS = "r14_detail_action_dev_import_complete_unaccepted_nonpromotable"
...
SOURCE_ROOT = (
    RUN_PARENT / "makehuman-cc0-detail-actions-r14-candidate-20260901a"
)
...
COMMANDLET = Path(__file__).with_name(
    "makehuman_cc0_detail_actions_r14_commandlet.py"
)

ATTEMPT_RE = re.compile(
    r"^makehuman-cc0-detail-actions-r14-ue57-dev-r1-[a-z0-9]"
    r"(?:[a-z0-9-]{0,62}[a-z0-9])?$"
)
SHA256_RE = re.compile(r"^[0-9a-f]{64}$")
PROJECT_FILE_NAME = "VistaMakeHumanCC0Import.uproject"
RESULT_NAME = "r14-detail-action-import-result.json"
RECEIPT_NAME = "r14-detail-action-import-receipt.json"
HOST_NAME = "r14-detail-action-dev-import-host-manifest.json"
STDOUT_NAME = "r14-detail-action-ue-stdout.log"
STDERR_NAME = "r14-detail-action-ue-stderr.log"
ENGINE_LOG_NAME = "r14-detail-action-ue-engine.log"
...
def validate_source(config: Config) -> tuple[
    ...
    raw, receipt_seal = read_file(
        config.source_receipt, "R14 worker receipt", maximum=MAX_JSON_BYTES
    )
    require(
        (receipt_seal.sha256, receipt_seal.size_bytes)
        == (SOURCE_RECEIPT_SHA256, SOURCE_RECEIPT_SIZE),
        "R14 worker receipt seal differs",
    )
    receipt = strict_json(raw, "R14 worker receipt")
    require(
        receipt.get("schema_version") == commandlet_contract.SOURCE_RECEIPT_SCHEMA
        and receipt.get("status")
        == "fresh_cc0_detail_action_candidates_roundtrip_verified"
        ...
        "R14 worker receipt contract differs",
    )
    artifacts = {
    ...
    for spec in commandlet_contract.CLIP_SPECS:
        ...
        require(
            ...
            "R14 worker artifact record differs: " + relative,
        )
        path = config.source_root / "artifacts" / relative
        _, seal = read_file(path, "R14 FBX " + spec["clip_id"])
        require(
            (seal.sha256, seal.size_bytes)
            == (spec["source_sha256"], spec["source_size_bytes"]),
            "R14 FBX seal differs: " + spec["clip_id"],
        )
...
def validate_plugin(
    ...
    tree = build_home.snapshot_tree(root, "fresh compiled R14 plugin")
    require(
...
def build_plan(
    ...
    *,
    ...
    _, commandlet = read_file(config.commandlet, "R14 import commandlet")
    plugin_tree = (
    ...
    report = seal_document(
        {
            ...
            "remaining_gates": (
                ...
                if ready
                    ...
                    "native BuildPlugin after R14 editor bridge compile",
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    pass
                process.wait()
            raise DetailActionImportError("UE R14 import timed out") from exc
...
def execute_import(plan: ImportPlan, acknowledgement: str) -> dict[str, Any]:
    ...
    copied_plugin = build_home.snapshot_tree(
        project / "Plugins/VistaPlayableHome", "copied compiled R14 plugin"
    )
    require(
        (
    ...
    commandlet = copy_file(
        plan.commandlet.path, input_root / "commandlet.py", "R14 commandlet"
    )
    require(
        (commandlet.sha256, commandlet.size_bytes)
        == (plan.commandlet.sha256, plan.commandlet.size_bytes),
        "copied R14 commandlet differs",
    )
    ...
    require(return_code == 0, "UE R14 import exited nonzero")
    result_raw, result_seal = read_file(
        plan.attempt_root / RESULT_NAME, "R14 commandlet result", maximum=MAX_JSON_BYTES
    )
    receipt_raw, receipt_seal = read_file(
        plan.attempt_root / RECEIPT_NAME,
        "R14 commandlet receipt",
        maximum=MAX_JSON_BYTES,
    )
    result = strict_json(result_raw, "R14 commandlet result")
    receipt = strict_json(receipt_raw, "R14 commandlet receipt")
    require(
