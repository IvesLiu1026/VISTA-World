...
readonly TRUSTED_SELF="${TRUSTED_ROOT}/bootstrap_vista_r8_native_builder_r2.sh"
readonly TRUSTED_BUILDER="${TRUSTED_ROOT}/vista_r8_native_builder.py"
readonly TRUSTED_SYSTEMD="${TRUSTED_ROOT}/systemd"
readonly BOOTSTRAP_LOCK="${TRUSTED_ROOT}/.bootstrap.lock"
...
assert_path_and_open_trusted_assets() {
  ...
  [[ -f "${BOOTSTRAP_LOCK}" && ! -L "${BOOTSTRAP_LOCK}" ]] || \
    fail 'bootstrap operation lock is not a regular file'
  [[ ! -L "${TRUSTED_SELF}" && ! -L "${TRUSTED_BUILDER}" && \
    ! -L "${TRUSTED_PHASE_A_UNIT}" && ! -L "${TRUSTED_PHASE_B_UNIT}" ]] || \
    fail 'trusted source symlink is forbidden'
  [[ "$(/usr/bin/stat -c '%u:%g:%h:%a:%s' -- "${BOOTSTRAP_LOCK}")" \
    == '0:0:1:600:0' ]] || fail 'bootstrap operation lock metadata differs'
  exec {BOOTSTRAP_LOCK_FD}<>"${BOOTSTRAP_LOCK}"
  /usr/bin/flock -n "${BOOTSTRAP_LOCK_FD}" || \
    fail 'another bootstrap operation holds the fixed lock'
  ...
  readonly BOOTSTRAP_LOCK_FD SELF_FD BUILDER_FD PHASE_A_UNIT_FD PHASE_B_UNIT_FD
}
...
verify_held_regular_path_binding() {
  local descriptor="$1" canonical_path="$2" uid="$3" gid="$4" mode="$5"
  local sha256="$6" size="$7" label="$8" path_identity held_identity
  [[ -f "${canonical_path}" && ! -L "${canonical_path}" ]] || \
    fail "${label} canonical path is not regular"
  path_identity="$(/usr/bin/stat -c '%d:%i' -- "${canonical_path}")" || \
    fail "cannot stat ${label} canonical path"
  held_identity="$(/usr/bin/stat -Lc '%d:%i' -- "$(held_path "${descriptor}")")" || \
    fail "cannot stat held ${label} identity"
  [[ "${path_identity}" == "${held_identity}" ]] || \
    fail "${label} canonical path identity differs"
...
verify_trusted_source_close_state() {
  ...
  assert_exact_inventory "${TRUSTED_ROOT}" \
    ...
    'vista_r8_native_builder.py' 'systemd'
  assert_exact_inventory "${TRUSTED_SYSTEMD}" "${PHASE_A_UNIT}" "${PHASE_B_UNIT}"
  verify_held_regular_path_binding "${SELF_FD}" "${TRUSTED_SELF}" 0 0 0500 \
    "${SELF_SOURCE_SHA256}" "${SELF_SOURCE_SIZE}" 'trusted live bootstrap'
  verify_held_regular_path_binding "${BUILDER_FD}" "${TRUSTED_BUILDER}" 0 0 0400 \
    "${BUILDER_SOURCE_SHA256}" "${BUILDER_SOURCE_SIZE}" 'trusted builder source'
  verify_held_regular_path_binding \
    "${PHASE_A_UNIT_FD}" "${TRUSTED_PHASE_A_UNIT}" 0 0 0400 \
    "${PHASE_A_UNIT_SHA256}" "${PHASE_A_UNIT_SIZE}" 'trusted phase A unit'
  verify_held_regular_path_binding \
    "${PHASE_B_UNIT_FD}" "${TRUSTED_PHASE_B_UNIT}" 0 0 0400 \
    "${PHASE_B_UNIT_SHA256}" "${PHASE_B_UNIT_SIZE}" 'trusted phase B unit'
}

verify_held_bootstrap_lock() {
  local held path_identity held_identity metadata
  held="$(held_path "${BOOTSTRAP_LOCK_FD}")"
  [[ -f "${held}" ]] || fail 'held bootstrap operation lock is not regular'
  metadata="$(/usr/bin/stat -Lc '%u:%g:%h:%a:%s' -- "${held}")" || \
    fail 'cannot stat held bootstrap operation lock'
  [[ "${metadata}" == '0:0:1:600:0' ]] || \
    fail 'held bootstrap operation lock metadata differs'
  path_identity="$(/usr/bin/stat -c '%d:%i' -- "${BOOTSTRAP_LOCK}")" || \
    fail 'cannot stat bootstrap operation lock path'
  held_identity="$(/usr/bin/stat -Lc '%d:%i' -- "${held}")" || \
    fail 'cannot stat held bootstrap operation lock identity'
  [[ "${path_identity}" == "${held_identity}" ]] || \
    fail 'bootstrap operation lock path identity differs'
}
...
assert_identity_prestate() {
  local group_by_name group_by_gid user_by_name user_by_uid
  group_by_name="$(/usr/bin/getent group "${BUILDER_NAME}" || true)"
  group_by_gid="$(/usr/bin/getent group "${BUILDER_GID}" || true)"
  user_by_name="$(/usr/bin/getent passwd "${BUILDER_NAME}" || true)"
  user_by_uid="$(/usr/bin/getent passwd "${BUILDER_UID}" || true)"

  if [[ -z "${group_by_name}" && -z "${group_by_gid}" &&
    -z "${user_by_name}" && -z "${user_by_uid}" ]]; then
    reject_subordinate_id_ranges
    return
  fi
  [[ -n "${group_by_name}" && "${group_by_name}" == "${group_by_gid}" &&
    -n "${user_by_name}" && "${user_by_name}" == "${user_by_uid}" ]] ||
    fail 'builder account/group prestate is mixed or colliding'
  validate_group 'false'
  validate_user 'false'
}

assert_framework_target_prestate() {
  if [[ ! -e "${LIBEXEC_ROOT}" && ! -L "${LIBEXEC_ROOT}" &&
    ! -e "${INPUT_ROOT}" && ! -L "${INPUT_ROOT}" &&
    ! -e "${STATE_ROOT}" && ! -L "${STATE_ROOT}" &&
    ! -e "${UNIT_ROOT}/${PHASE_A_UNIT}" &&
    ! -L "${UNIT_ROOT}/${PHASE_A_UNIT}" &&
    ! -e "${UNIT_ROOT}/${PHASE_B_UNIT}" &&
    ! -L "${UNIT_ROOT}/${PHASE_B_UNIT}" ]]; then
    return
  fi
  ...
  [[ ! -e "${PHASE_A_FINAL}" && ! -L "${PHASE_A_FINAL}" ]] ||
    fail 'framework reconcile prestate contains phase A publication'
  [[ ! -e "${PHASE_B_FINAL}" && ! -L "${PHASE_B_FINAL}" ]] ||
    fail 'framework reconcile prestate contains phase B publication'
}
