...
class PlayableHomeCompilerTests(unittest.TestCase):
    ...
    ) -> compiler.PlayableHomeContractError:
    ...
    def test_two_resolutions_and_reversed_event_inputs_are_identical(self) -> None:
        ...
        second = compiler.compile_build_plan(
            copy.deepcopy(self.house), list(reversed(copy.deepcopy(self.events)))
        )
        self.assertEqual(
            compiler.canonical_json_bytes(first), compiler.canonical_json_bytes(second)
        )
    ...
    def test_plan_is_closed_schema_valid_and_has_exact_bindings(self) -> None:
        ...
        self.assertEqual(
            plan["unreal"]["navigation_bounds_cm"],
            {"min_cm": [-650, -400, 0], "max_cm": [650, 800, 300]},
        )
        primary = next(
            event for event in plan["event_plans"] if event["event_id"] == "mmg_001"
        )
        ...
        self.assertEqual(
            primary["source"]["public_reference"], "vista://assist-step/mmg_001"
        )
    ...
    def test_room_local_transforms_resolve_to_world_centimeters(self) -> None:
        ...
        self.assertEqual(
            coffee_table["world_transform_cm"]["location_cm"], [-400, -170, 0]
        )
        ...
        self.assertEqual(player["world_transform_cm"]["location_cm"], [0, -130, 10])

    def test_rotation_composition_is_deterministic(self) -> None:
        changed = copy.deepcopy(self.house)
        living = next(
            room for room in changed["rooms"] if room["kind"] == "living_room"
        )
        ...
        table = next(
            ...
            if entity["category"] == "coffee_table"
        )
    ...
    def test_event_transform_operations_are_resolved(self) -> None:
        plan = compiler.compile_build_plan(self.house, self.events)
        event = next(
            item for item in plan["event_plans"] if item["event_id"] == "mmg_044"
        )
        operation = next(
            item for item in event["operations"] if item["op"] == "set_transform"
        )
        self.assertEqual(
            operation["world_transform_cm"]["location_cm"], [-435, -170, 50]
        )
    ...
    def test_build_plan_unknown_field_and_digest_mismatch_fail_closed(self) -> None:
        ...
        self.assert_contract_error(
            "VISTA_HOME_SCHEMA_INVALID", lambda: compiler.validate_build_plan(changed)
        )

        changed = copy.deepcopy(plan)
        changed["plan_id"] = "home.r1@vista_playable_home_r2"
        self.assert_contract_error(
            "VISTA_HOME_DIGEST_MISMATCH", lambda: compiler.validate_build_plan(changed)
        )

    def test_build_plan_target_and_npc_action_fail_closed(self) -> None:
        plan = compiler.compile_build_plan(self.house, self.events)
        changed = copy.deepcopy(plan)
        changed["event_plans"][0]["participating_entity_ids"][0] = (
            "home.r1/room.entry_hall/entity.unknown.99"
        )
        changed = compiler.seal_document(changed)
        self.assert_contract_error(
            "VISTA_HOME_BUILD_PLAN_TARGET_UNKNOWN",
            lambda: compiler.validate_build_plan(changed),
        )
    ...
    def test_cli_validate_and_compile_output(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with redirect_stdout(stdout):
                code = compiler.main(
                    [
                        "validate",
                        "--house",
                        str(PACK_ROOT / "house.json"),
                        "--events-dir",
                        str(PACK_ROOT / "events"),
                    ]
                )
            ...
            self.assertEqual(
                output.read_bytes(), compiler.canonical_json_bytes(plan) + b"\n"
            )
