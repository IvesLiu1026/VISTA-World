...
```text
/root/vista-r8-native-builder-bootstrap-r2/       root:root 0555
  .bootstrap.lock                                root:root 0600 (zero bytes)
  bootstrap_vista_r8_native_builder_r2.sh            root:root 0500
  vista_r8_native_builder.py                      root:root 0400
  systemd/                                        root:root 0555
    vista-r8-native-builder-r2-phase-a.service       root:root 0400
    vista-r8-native-builder-r2-phase-b.service       root:root 0400

/root/vista-r8-native-builder-bootstrap-input-r2/ root:root 0700
...
/usr/local/libexec/vista-r8-native-builder-r2/    root:root 0555
  vista_r8_native_builder.py                      root:root 0444

/etc/vista-r8-native-builder-r2/                  root:root 0555
  source.bundle                                   root:root 0444
  phase-a-request.json                            root:root 0444
  # appended only after phase A review:
  phase-b-request.json                            root:root 0444

/var/lib/vista-r8-native-builder-r2/              root:root 0555
...
/etc/systemd/system/
  vista-r8-native-builder-r2-phase-a.service         root:root 0644
  vista-r8-native-builder-r2-phase-b.service         root:root 0644
```

The fixed `.bootstrap.lock` is opened read/write, validated by path and held
descriptor, and acquired with nonblocking `flock` before any operation. It is
held until the final close-state gate, so framework, Phase A input, and Phase B
input operations cannot race one another.

The failed R1 roots remain append-only evidence:
...
R1 evidence is never reset, moved, replaced, reconciled, or modified. Both R2
units explicitly hide all five roots with `InaccessiblePaths=-...`; no R2
builder, launcher, or installer opens an R1 native-builder publication path.
Unrelated approved `r1`-suffixed engine, host-runtime, BuildPlugin, parent-seal,
stage, attempt, and evidence authorities retain their own versioned contracts;
they are not aliases of these five failed native-builder roots.
...
`/root/vista-r8-native-builder-bootstrap-r2/bootstrap_vista_r8_native_builder_r2.sh`.
...
Before creating the group, user, any directory, lock, builder, or unit, the
framework operation performs one zero-write preflight. The group and passwd
name/numeric lookups must be either all absent or both fully exact; a UID/GID/
name collision fails before `groupadd`. The five R2 filesystem targets must be
either all lexically absent, or one complete exact framework with empty input,
lock-only slots, and no publication. Mixed, dirty, or interrupted framework
prefixes are preserved as evidence and are never implicitly resumed or
repaired.
...
The separately reviewed activation wrapper must first require adequate free
disk headroom, an absent R2 publication, a fresh inactive R2 manager/cgroup
state, and the preserved failed R1 unit/evidence state. The sole daemon reload
must not reset, move, or alter the R1 state.
...
```bash
/usr/bin/bash -n tools/admin/bootstrap_vista_r8_native_builder_r2.sh
systemd-analyze verify \
  tools/admin/systemd/vista-r8-native-builder-r2-phase-a.service \
  tools/admin/systemd/vista-r8-native-builder-r2-phase-b.service
...
The R1 ceremony installed its reviewed framework and Phase A inputs, then
started Phase A exactly once. That run failed closed at
`TRACE_PATH_DRIFT: trace host file[21]`; Phase A published nothing and Phase B
never started. The R1 unit failure state, installed files, locks, journal, and
empty publication boundary are retained as evidence. R2 is a new namespace,
not a repair or reconciliation of R1. Its source, units, inputs, state root,
initial-bootstrap launcher, and installer all use R2 paths; no R2 privileged
installation, daemon reload, service start, compilation, or publication is
claimed by this source milestone.
